...
function addElementToLayer(el, layer, xform) {
	...
	switch(el['#name']) {
		...
		case 'text':	 return addTextToLayer(el, layer, xform);
...
function addTextToLayer(el, layer) {
    const c = attrs(el, 'transform');
    const t = c.transform.match(/(\d+(\.\d+)?)/g);
    const m4 = t[4];
    const m5 = t[5];
    const geo = {
        type: 'Point',
        coordinates: [parseFloat(m4), parseFloat(m5)]
    };
    addDebugId(geo, el, `${el._}`);
    layer.push(geo);
}
